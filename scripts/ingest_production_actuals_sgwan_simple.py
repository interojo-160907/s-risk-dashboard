...
def main() -> int:
    ...
    ap.add_argument("--qty-col", default="양품수량", help="수량 컬럼(기본: 양품수량)")
    ...
    logger.info(
        "start | input=%s | sheets=%s | status=%s | qty_col=%s | output=%s",
        in_path,
        args.sheets,
        args.status,
        args.qty_col,
    ...
    required = ["생산일자", "공정코드", "품목코드", args.qty_col, "상태"]
    ...
    out = pd.DataFrame(
        {
            ...
            PROD_COLS.생산수량: pd.to_numeric(raw[args.qty_col], errors="coerce"),
        }
    )
    ...
    out[PROD_REQUIRED_COLS].to_csv(out_path, index=False, encoding="utf-8-sig")
